...
const userSchema = new Schema({
  ...
  // chatBoxes: [{ type: mongoose.Types.ObjectId, ref: 'ChatBox' }], // 有global的chatBoxes，又記得各自的clients
});
...
// static: let everyone seeing the same thing
// public: ./public/index.html => in localhost 8000
  ...
  client.on('message', async function incoming(message) {
    ...
    switch (type) {
      ...
      case 'CHAT': {
        ...
        chatBox.messages.map(({ sender: { name }, body }) => (console.log({
          name,
          body,
        })))
        client.sendEvent({
